...
type GetPossibleTravelsUseCaseUseCaseImplTestSuite struct {
	suite.Suite
	ctx                  	 context.Context
	logger               	 *logrus_config.LogrusLogger
	createTravelRepository 	 *GetPossibleTravelsUseCase
}

func (suite *GetPossibleTravelsUseCase) SetupTest() {
	...
	useCaseRepository := NewGetPossibleTravelsUseCase(
		ctx,
		logger,
		busPGSQLRepository,
		cityPGSQLRepository,
